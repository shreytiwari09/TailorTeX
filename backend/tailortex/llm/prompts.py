...
- Quantify the result by KEEPING a number that is already in this entry or in evidence you cite. Rule 1 still wins: a number you can't source is a rejected operation. If neither the entry nor the evidence states a result, end on what was built or done; never add an outcome that isn't stated. Never estimate, round or combine figures.
...
- Use a number ONLY if the answer contains it, written the same way. State a result only if the answer states one. If it doesn't, end on what was built or done: "enabling successful classification" from "it worked well" is an invention. Never estimate, round or combine figures.
...
- An answer with no target and no new_project mark is a plain reply, and you decide where it belongs. If the work it describes was done inside a job or project already on the resume, add a bullet to that entry (an "add" operation). If it describes a separate project that is not on the resume, add it to "projects" with the name, dates and technologies exactly as the answer gives them, and leave a field empty if the answer doesn't say. A reply that names the project, says what it did and what it used is enough: write the bullet(s) rather than asking for more. If the reply names no project and you can't tell which entry it belongs to, do not guess: write no operation and add a followup asking what the project is called and when it was, or which entry it belongs to.
- An answer marked new_project describes a separate project that is not on the resume. Do NOT write an operation for it. Add one item to "projects" instead: {{"answer":"ans1","name":"...","dates":"...","tech":["..."],"bullets":["...","..."]}}, with one to four bullets in the same what / how / result shape: one strong bullet is enough when that is all the answer supports. The project's name and technologies are given; use only those and what the answer says. Add a followup instead only when the answer doesn't say what was built or what it was built with.
